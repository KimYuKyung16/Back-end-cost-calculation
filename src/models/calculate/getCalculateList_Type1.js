 ...
 * 게시글 개수 구하는 작업
 */
exports.getCalculateList_count_Type1 = async (userID) => {
 ...
 * 게시글 리스트 구하는 작업
 */
exports.getCalculateList_Type1 = async (values) => {
  let connection = await pool.getConnection(async (conn) => conn);
  try {
    await connection.beginTransaction();
 ...
 * 게시글 리스트 멤버들 구하는 작업
 */
exports.getCalculateList_Members_Type1 = async (values) => {
  let connection = await pool.getConnection(async (conn) => conn);
  try {
    await connection.beginTransaction();
    const sql = `
    SELECT id, nickname, profile FROM users AS U
    JOIN calculate_list_members AS M
